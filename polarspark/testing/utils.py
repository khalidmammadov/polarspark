...
from polarspark.sql.types import StructType, StructField
from polarspark.sql.functions import col, when
...
def assertSchemaEqual(
    ...
    Examples
    --------
    >>> from polarspark.sql.types import StructType, StructField, ArrayType, IntegerType, DoubleType
...
if TYPE_CHECKING:
    import pandas
    import polarspark.pandas


def assertDataFrameEqual(
    actual: Union[DataFrame, "pandas.DataFrame", "polarspark.pandas.DataFrame", List[Row]],
    expected: Union[DataFrame, "pandas.DataFrame", "polarspark.pandas.DataFrame", List[Row]],
    ...
    >>> import polarspark.pandas as ps
    ...
    *** expected ***
    ...
    >>> from polarspark.sql.types import StructType, StructField, StringType, LongType
    ...
    *** expected ***
    ...
    try:
        ...
        import polarspark.pandas as ps
        import pandas as pd
        from polarspark.testing.pandasutils import PandasOnSparkTestUtils
    ...
    if has_pandas:
        ...
        from polarspark.sql.utils import get_dataframe_class
...
def _test() -> None:
    ...
    import polarspark.testing.utils

    globs = polarspark.testing.utils.__dict__.copy()
    spark = SparkSession.builder.master("local[4]").appName("testing.utils tests").getOrCreate()
    globs["spark"] = spark
    (failure_count, test_count) = doctest.testmod(
        polarspark.testing.utils,
        globs=globs,
