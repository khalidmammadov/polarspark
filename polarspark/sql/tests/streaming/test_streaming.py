...
from polarspark.errors import PySparkValueError
...
class StreamingTestsMixin:
    ...
    def test_streaming_with_temporary_view(self):
        ...
        with self.table("input_table", "this_query"):
            ...
            self.spark.sql("select * from input_table").show()
            df = self.spark.readStream.table("input_table")
            self.assertTrue(df.isStreaming)
            # Create a temp view
...
if __name__ == "__main__":
    import unittest
    from polarspark.sql.tests.streaming.test_streaming import *  # noqa: F401
