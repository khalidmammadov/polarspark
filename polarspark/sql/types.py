...
from polarspark.errors import PySparkNotImplementedError, PySparkTypeError, PySparkValueError
...
class DataType:
    ...
    def fromDDL(cls, ddl: str) -> "DataType":
        ...
        ddl : str
            ...
            :class:`polarspark.sql.types.DataType.simpleString`, except that top level struct
...
class ArrayType(DataType):
    ...
    Examples
    --------
    >>> from polarspark.sql.types import ArrayType, StringType, StructField, StructType
...
class MapType(DataType):
    ...
    Examples
    --------
    >>> from polarspark.sql.types import IntegerType, FloatType, MapType, StringType
...
class StructField(DataType):
    ...
    Examples
    --------
    >>> from polarspark.sql.types import StringType, StructField
...
class StructType(DataType):
    ...
    def add(
        ...
        Examples
        --------
        >>> from polarspark.sql.types import IntegerType, StringType, StructField, StructType
    ...
    def fieldNames(self) -> List[str]:
        ...
        Examples
        --------
        >>> from polarspark.sql.types import StringType, StructField, StructType
...
def _parse_datatype_string(s: str) -> DataType:
    ...
    """
    try:
        return _data_type_from_ddl(s)
    except Exception as e:
        try:
            # For backwards compatibility, "fieldname: datatype, fieldname: datatype" case.
            return _data_type_from_ddl("struct<%s>" % s.strip())
...
def _make_type_verifier(
    ...
    Traceback (most recent call last):
        ...
    polarspark.errors.exceptions.base.PySparkValueError:...
    >>> _make_type_verifier(ArrayType(ShortType()))(list(range(3)))
    >>> _make_type_verifier(ArrayType(StringType()))(set()) # doctest: +IGNORE_EXCEPTION_DETAIL
    Traceback (most recent call last):
        ...
    polarspark.errors.exceptions.base.PySparkTypeError:...
...
def _to_polars_type(data_type: Union[DataType, str]) -> pl.DataType:
    ...
    if type_str == "decimal":
        _, parsed_args = _parse_type(data_type)
        return pl.Decimal(*parsed_args)
...
def _parse_type(type_str: str) -> tuple[str, list]:
    """
    Parse a type string like 'decimal(12,3)' into a tuple of parsed parts.
    Example:
    ...
    if args:
        ...
        arg_list = []
        for a in _split_top_level_commas(args):
            a = a.strip()
            if a.isdigit():
...
def _parse_ddl_fields(fields: list[str]) -> list[StructField]:
    ...
    for f in fields:
        m = pattern.match(f)
        if not m:
            raise ValueError(f"Invalid field format: {f!r}")
        name, type_str = m.groups()
        if name in seen:
            raise ValueError(f"Duplicate field name: {name}")
        result.append(StructField(name, _data_type_from_ddl(type_str)))

    return result


def _data_type_from_ddl(ty: str) -> DataType:
    """
    Parse strings like below into DataType.
    Example:
        'decimal(12,3)' -> 'decimal', [12, 3]
        'int' -> 'int', []
        'array<int>' -> 'array', ['int']
    ...
    def to_type(_ty):
        ...
        if _ty == "struct":
            return StructType(_parse_ddl_fields(_args))

        # Covert array<int> etc.
        _args = (to_type(arg) or arg for arg in _args)
