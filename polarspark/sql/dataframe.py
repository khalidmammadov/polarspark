...
# from polarspark.serializers import BatchedSerializer, CPickleSerializer, UTF8Deserializer
...
if TYPE_CHECKING:
    ...
    # from polarspark.sql.observation import Observation
...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def __init__(
        ...
        transformer: Union[
            Callable[[LazyFrame], LazyFrame], Callable[[], Generator[LazyFrame, None, None]]
        ],
    ...
    def schema(self) -> StructType:
        """Returns the schema of this :class:`DataFrame` as a :class:`polarspark.sql.types.StructType`.
    ...
    def _repr_html_(self) -> Optional[str]:
        ...
        """
        # TODO: check LazyFrame's repr_html
        return None
    ...
    def checkpoint(self, eager: bool = True) -> "DataFrame":
        ...
        """
        # Nothing to checkpoint, so return just self
        return self
    ...
    def hint(
        ...
        ...
        """
        raise NotImplementedError("Hints not implemented yet")
    ...
    def collect(self) -> List[Row]:
        ...
        >>> from polarspark.sql.functions import upper
        >>> df = spark.createDataFrame([(14, "Tom"), (23, "Alice"), (16, "Bob")], ["age", "name"])
    ...
    def tail(self, num: int) -> List[Row]:
        ...
        return self._to_df(transformer).collect()
    ...
    def distinct(self) -> "DataFrame":
        ...
        def transformer(ldf):
            return ldf.unique(subset=None, maintain_order=False, keep="any")
    ...
    def sample(  # type: ignore[misc]
        ...
        def transformer(ldf):
            pdf: pl.DataFrame = ldf.collect()
            n = len(pdf) * fraction
            return pdf.sample(n, seed=seed).lazy()
    ...
    def to(self, schema: StructType) -> "DataFrame":
        ...
        * Fail if the nullability is not compatible. For example, the column and/or inner field
        ...
        Examples
        --------
        >>> from polarspark.sql.types import StructField, StringType
    ...
    def alias(self, alias: str) -> "DataFrame":
        ...
        Examples
        --------
        >>> from polarspark.sql.functions import col, desc
        >>> df = spark.createDataFrame(
        ...     [(14, "Tom"), (23, "Alice"), (16, "Bob")], ["age", "name"])
        ...
        return self._to_df(lambda ldf: ldf, alias=alias)
    ...
    def join(
        ...
        Traceback (most recent call last):
        ...
        polarspark.errors.exceptions.captured.AnalysisException: Column name#0 are ambiguous...
        ...
        **kwargs: Any,
        ...
        for c in cols:
            if isinstance(c, Column):
                ...
                raise ValueError(
                    "NOT_COLUMN_STR_INT. Grouping column must be" "either Column, str or int"
                )
        ...
        def transformer(ldf):
            return ldf.sort(by[0], *by[1:], descending=desc)
        ...
        def transformer(ldf):
            if isinstance(condition, str):
                if condition.lower() == "false":
                    ...
                    return ldf.filter(condition)
            return ldf.filter(condition._expr)
    ...
    def observe(
        ...
        * It will report the value of the defined aggregate columns as soon as we reach a completion
        ...
        Python's :class:`~polarspark.sql.streaming.StreamingQueryListener`,
        ...
        >>> from polarspark.sql.functions import col, count, lit, max
    ...
    def dropDuplicates(self, subset: Optional[List[str]] = None) -> "DataFrame":
        ...
        def transformer(ldf):
            return ldf.unique(subset=subset, maintain_order=False, keep="any")
    ...
    def dropna(
        ...
        if subset and not isinstance(subset, (str, list, tuple)):
            raise PySparkTypeError(
        ...
        def transformer(ldf):
            if subset:
                return ldf.drop_nulls(subset)
            return ldf.drop_nulls()
            ...
            def transformer(ldf):
                ...
                for c in _df.columns:
                    if c in value:
                        ...
                        sers[c] = _df[c]
                return pl.DataFrame(sers).lazy()

            return self._to_df(transformer)

        def _fill(ldf, sub, val):
            # Fill Nulls for given subset of columns
            return ldf.with_columns(pl.col(sub).fill_null(val))

        def transformer(ldf):
            def subs(t):
                # Get subset of columns using type selectors
                pl_type = to_polars_selector(t)
                cols = pl.selectors.by_dtype(pl_type)
                return ldf.select(cols).collect_schema().names()

            if subset is None:
                if isinstance(value, (int, float)) and not isinstance(value, bool):
                    return _fill(_fill(ldf, subs("int"), int(value)), subs("float"), value)
                else:
                    return _fill(ldf, subs(str(type(value).__name__)), value)
            ...
            if isinstance(value, (int, float)) and not isinstance(value, bool):
                return _fill(_fill(ldf, ff(subs("int")), int(value)), ff(subs("float")), value)
            else:
                return _fill(ldf, ff(subs(str(type(value).__name__))), value)
    ...
    def withColumn(self, colName: str, col: Column) -> "DataFrame":
        ...
        def transformer(ldf):
            return ldf.with_columns(col._expr.alias(colName))  # noqa
    ...
    def withColumnRenamed(self, existing: str, new: str) -> "DataFrame":
        ...
        def transformer(ldf):
            return ldf.rename({existing: new})
    ...
    def drop(self, *cols: "ColumnOrName") -> "DataFrame":  # type: ignore[misc]
        ...
        Traceback (most recent call last):
        ...
        polarspark.errors.exceptions.captured.AnalysisException: [AMBIGUOUS_REFERENCE] Reference...
    ...
    def _trans_gen(self):
        if self._parent:
            yield from self._parent._trans_gen()
        yield self._transformer
...
def _test() -> None:
    ...
    import polarspark.sql.dataframe

    globs = polarspark.sql.dataframe.__dict__.copy()
    spark = SparkSession.builder.master("local[4]").appName("sql.dataframe tests").getOrCreate()
    globs["spark"] = spark
    (failure_count, test_count) = doctest.testmod(
        polarspark.sql.dataframe,
        globs=globs,
