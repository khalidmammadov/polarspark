...
if TYPE_CHECKING:
    ...
    # from polarspark.sql.udf import UDFRegistration
    # from polarspark.sql.udtf import UDTFRegistration
    # from polarspark.sql.datasource import DataSourceRegistration

    # Running MyPy type checks will always require pandas and
    # other dependencies so importing here is fine.
    # from polarspark.sql.connect.client import SparkConnectClient
...
class SparkSession(SparkConversionMixin):
    ...
    class Builder:
        ...
        def config(
            ...
            *,
            ...
            <polarspark.sql.session.SparkSession.Builder...
        ...
        #         from polarspark.sql.connect.session import SparkSession as RemoteSparkSession
        #
    ...
    def conf(self) -> RuntimeConfig:
        ...
        Returns
        -------
        :class:`polarspark.sql.conf.RuntimeConfig`

        Examples
        --------
        >>> spark.conf
        <polarspark...RuntimeConf...>
    ...
    def range(
        ...
        """
        Create a :class:`DataFrame` with single :class:`polarspark.sql.types.LongType` column named
    ...
    def _inferSchemaFromList(
        ...
        Returns
        -------
        :class:`polarspark.sql.types.StructType`
        """
        if not data:
    ...
    def _create_shell_session() -> "SparkSession":
        """
        Initialize a :class:`SparkSession` for a polarspark shell session. This is called from
    ...
    def createDataFrame(  # type: ignore[misc]
        ...
        schema : :class:`polarspark.sql.types.DataType`, str or list, optional
            a :class:`polarspark.sql.types.DataType` or a datatype string or a list of
            column names, default is None. The data type string format equals to
            :class:`polarspark.sql.types.DataType.simpleString`, except that top level struct type can
            omit the ``struct<>``.
            ...
            When ``schema`` is :class:`polarspark.sql.types.DataType` or a datatype string, it must
            match the real data, or an exception will be thrown at runtime. If the given schema is
            not :class:`polarspark.sql.types.StructType`, it will be wrapped into a
            :class:`polarspark.sql.types.StructType` as its only field, and the field name will be
        ...
        Notes
        -----
        Usage with `spark.sql.execution.arrow.polarspark.enabled=True` is experimental.
    ...
    def sql(
        ...
        >>> from polarspark.sql.functions import create_map
        >>> spark.sql(
        ...
        >>> from polarspark.sql.functions import array
        >>> spark.sql(
    ...
    def streams(self) -> "StreamingQueryManager":
        ...
        --------
        >>> spark.streams
        <polarspark...StreamingQueryManager object ...>
