...
from plspark import copy_func
from plspark.context import SparkContext
from plspark.errors import PySparkAttributeError, PySparkTypeError, PySparkValueError
from plspark.sql.types import DataType
from plspark.sql.utils import get_active_spark_context

if TYPE_CHECKING:
    from plspark.sql._typing import ColumnOrName, LiteralType, DecimalLiteral, DateTimeLiteral
    from plspark.sql.window import WindowSpec
...
class Column:
    ...
    def dropFields(self, *fieldNames: str) -> "Column":
        ...
        --------
        >>> from plspark.sql import Row
        >>> from plspark.sql.functions import col, lit
        >>> df = spark.createDataFrame([
    ...
    def alias(self, *alias: str, **kwargs: Any) -> "Column":
        ...
        metadata: dict
            ...
            corresponding :class:`StructField <plspark.sql.types.StructField>` (optional, keyword
            only argument)
    ...
    def cast(self, dataType: Union[DataType, str]) -> "Column":
        ...
        Examples
        --------
        >>> from plspark.sql.types import StringType
        >>> df = spark.createDataFrame(
        ...      [(2, "Alice"), (5, "Bob")], ["age", "name"])
    ...
    def over(self, window: "WindowSpec") -> "Column":
        ...
        ... )
        >>> from plspark.sql.functions import rank, min, desc
        >>> df = spark.createDataFrame(
        ...      [(2, "Alice"), (5, "Bob")], ["age", "name"])
        >>> df.withColumn(
...
def _test() -> None:
    import doctest
    from plspark.sql import SparkSession
    import plspark.sql.column

    globs = plspark.sql.column.__dict__.copy()
