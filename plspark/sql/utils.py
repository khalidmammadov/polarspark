...
from plspark.errors import (  # noqa: F401
    AnalysisException,
    ParseException,
...
from plspark.errors.exceptions.captured import CapturedException  # noqa: F401
# from plspark.find_spark_home import _find_spark_home
...
if TYPE_CHECKING:
    ...
    from plspark.sql.window import Window
    # from plspark.pandas._typing import IndexOpsLike, SeriesOrIndex
...
def is_timestamp_ntz_preferred() -> bool:
    ...
    """
    if is_remote():
        from plspark.sql.connect.session import SparkSession as ConnectSparkSession
...
def is_remote() -> bool:
    ...
    Examples
    --------
    >>> from plspark.sql import is_remote
    >>> is_remote()
    False
    """
...
def try_remote_functions(f: FuncT) -> FuncT:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from plspark.sql.connect import functions
...
def try_remote_avro_functions(f: FuncT) -> FuncT:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from plspark.sql.connect.avro import functions
...
def try_remote_protobuf_functions(f: FuncT) -> FuncT:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from plspark.sql.connect.protobuf import functions
...
def try_remote_window(f: FuncT) -> FuncT:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from plspark.sql.connect.window import Window  # type: ignore[misc]
...
def try_remote_windowspec(f: FuncT) -> FuncT:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from plspark.sql.connect.window import WindowSpec
...
def try_remote_session_classmethod(f: FuncT) -> FuncT:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        if is_remote() and "PYSPARK_NO_NAMESPACE_SHARE" not in os.environ:
            from plspark.sql.connect.session import SparkSession  # type: ignore[misc]
...
def plspark_column_op(
    ...
    """
    from plspark.pandas.base import column_op
    from plspark.sql.column import Column as PySparkColumn
    from plspark.pandas.data_type_ops.base import _is_extension_dtypes

    if is_remote():
        from plspark.sql.connect.column import Column as ConnectColumn
...
def get_dataframe_class() -> Type["DataFrame"]:
    from plspark.sql.dataframe import DataFrame as PySparkDataFrame

    if is_remote():
        from plspark.sql.connect.dataframe import DataFrame as ConnectDataFrame
...
def get_window_class() -> Type["Window"]:
    from plspark.sql.window import Window as PySparkWindow

    if is_remote():
        from plspark.sql.connect.window import Window as ConnectWindow
