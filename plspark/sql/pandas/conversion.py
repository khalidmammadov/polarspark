...
from plspark.errors.exceptions.captured import unwrap_spark_exception
# from plspark.loose_version import LooseVersion
# from plspark.rdd import _load_from_socket
from plspark.sql.pandas.serializers import ArrowCollectSerializer
from plspark.sql.pandas.types import _dedup_names
from plspark.sql.types import ArrayType, MapType, TimestampType, StructType, DataType, _create_row
from plspark.sql.utils import is_timestamp_ntz_preferred
from plspark.traceback_utils import SCCallSiteSync
from plspark.errors import PySparkTypeError
...
def schema_from_pandas(pdf: "PandasDataFrameLike", schema: Optional[Union[StructType, List[str]]] = None):
    import pyarrow as pa
    from plspark.sql.pandas.types import from_arrow_type
...
class SparkConversionMixin:
    ...
    def _create_from_pandas_with_arrow(
        ...
        schema_from_pandas(pdf, schema)
        # from plspark.sql.pandas.serializers import ArrowStreamPandasSerializer
        # from plspark.sql.types import TimestampType
        # from plspark.sql.pandas.types import (
        #     from_arrow_type,
        #     to_arrow_type,
        #     _deduplicate_field_names,
        # )
        # from plspark.sql.pandas.utils import (
        #     require_minimum_pandas_version,
        #     require_minimum_pyarrow_version,
        # )
        #
        ...
        # step = 10000 #self._jconf.arrowMaxRecordsPerBatch()
        ...
        # FIX, looks like pl.from_pandas clones the data, check back how to use Arrow data directly instead
        df = DataFrame(pl.from_pandas(pdf).lazy(), self)
        df._schema = schema
        return df
