...
from plspark import copy_func, _NoValue
from plspark._globals import _NoValueType
from plspark.context import SparkContext
from plspark.errors import PySparkTypeError, PySparkValueError
from plspark.rdd import (
    RDD,
    _load_from_socket,
    _local_iterator_from_socket,
)
# from plspark.serializers import BatchedSerializer, CPickleSerializer, UTF8Deserializer
from plspark.storagelevel import StorageLevel
from plspark.traceback_utils import SCCallSiteSync
from plspark.sql.column import Column#, _to_seq, _to_list, _to_java_column
from plspark.sql.readwriter import DataFrameWriter, DataFrameWriterV2
# from plspark.sql.streaming import DataStreamWriter
...
if TYPE_CHECKING:
    from plspark._typing import PrimitiveType
    # from plspark.pandas.frame import DataFrame as PandasOnSparkDataFrame
    ...
    from plspark.sql.group import GroupedData
    # from plspark.sql.observation import Observation
...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def rdd(self) -> "RDD[Row]":
        """Returns the content as an :class:`plspark.RDD` of :class:`Row`.
    ...
    def stat(self) -> "DataFrameStatFunctions":
        ...
        Examples
        --------
        >>> import plspark.sql.functions as f
    ...
    def schema(self) -> StructType:
        """Returns the schema of this :class:`DataFrame` as a :class:`plspark.sql.types.StructType`.
    ...
    def collect(self) -> List[Row]:
        ...
        >>> from plspark.sql.functions import upper
        >>> df = spark.createDataFrame([(14, "Tom"), (23, "Alice"), (16, "Bob")], ["age", "name"])
    ...
    def sampleBy(
        ...
        Examples
        --------
        >>> from plspark.sql.functions import col
    ...
    def to(self, schema: StructType) -> "DataFrame":
        ...
        * Fail if the nullability is not compatible. For example, the column and/or inner field
        ...
        Examples
        --------
        >>> from plspark.sql.types import StructField, StringType
    ...
    def alias(self, alias: str) -> "DataFrame":
        ...
        Examples
        --------
        >>> from plspark.sql.functions import col, desc
        >>> df = spark.createDataFrame(
        ...     [(14, "Tom"), (23, "Alice"), (16, "Bob")], ["age", "name"])
    ...
    def join(
        ...
        Traceback (most recent call last):
        ...
        plspark.errors.exceptions.captured.AnalysisException: Column name#0 are ambiguous...
    ...
    def observe(
        ...
        * It will report the value of the defined aggregate columns as soon as we reach a completion
        ...
        Python's :class:`~plspark.sql.streaming.StreamingQueryListener`,
        ...
        >>> from plspark.sql.functions import col, count, lit, max
        >>> from plspark.sql import Observation
        >>> df = spark.createDataFrame([(2, "Alice"), (5, "Bob")], schema=["age", "name"])
        ...
        >>> from plspark.sql.streaming import StreamingQueryListener
    ...
    def drop(self, *cols: "ColumnOrName") -> "DataFrame":  # type: ignore[misc]
        ...
        Traceback (most recent call last):
        ...
        plspark.errors.exceptions.captured.AnalysisException: [AMBIGUOUS_REFERENCE] Reference...
    ...
    def pandas_api(
        ...
        See Also
        --------
        plspark.pandas.frame.DataFrame.to_spark
        ...
        16     Bob
        """
        from plspark.pandas.namespace import _get_index_map
        from plspark.pandas.frame import DataFrame as PandasOnSparkDataFrame
        from plspark.pandas.internal import InternalFrame
...
def _test() -> None:
    ...
    import plspark.sql.dataframe

    globs = plspark.sql.dataframe.__dict__.copy()
    spark = SparkSession.builder.master("local[4]").appName("sql.dataframe tests").getOrCreate()
    globs["spark"] = spark
    (failure_count, test_count) = doctest.testmod(
        plspark.sql.dataframe,
        globs=globs,
