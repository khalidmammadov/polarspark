...
from plspark import SparkContext
from plspark.errors import PySparkTypeError, PySparkValueError
from plspark.sql.column import Column#, _to_java_column, _to_seq, _create_column_from_literal
from plspark.sql.dataframe import DataFrame
from plspark.sql.types import ArrayType, DataType, StringType, StructType, _from_numpy_type

# Keep UserDefinedFunction import for backwards compatible import; moved in SPARK-22409
# from plspark.sql.udf import UserDefinedFunction, _create_py_udf  # noqa: F401
# from plspark.sql.udtf import AnalyzeArgument, AnalyzeResult  # noqa: F401
# from plspark.sql.udtf import OrderingColumn, PartitioningColumn  # noqa: F401
# from plspark.sql.udtf import UserDefinedTableFunction, _create_py_udtf

# Keep pandas_udf and PandasUDFType import for backwards compatible import; moved in SPARK-28264
# from plspark.sql.pandas.functions import pandas_udf, PandasUDFType  # noqa: F401

from plspark.sql.utils import (
    to_str as _to_str,
...
if TYPE_CHECKING:
    from plspark.sql._typing import (
        ColumnOrName,
        ColumnOrName_,
        DataTypeOrString,
...
def _invoke_function(name: str, *args: Any) -> Column:
    ...
    and wraps the result with :class:`~plspark.sql.Column`.
    """
...
def lit(col: Any) -> Column:
    """
    Creates a :class:`~plspark.sql.Column` of literal value.
    ...
    Parameters
    ----------
    col : :class:`~plspark.sql.Column`, str, int, float, bool or list, NumPy literals or ndarray.
...
def col(col: str) -> Column:
    """
    Returns a :class:`~plspark.sql.Column` based on the given column name.
...
def asc(col: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    col : :class:`~plspark.sql.Column` or str
    ...
    >>> from plspark.sql.functions import asc
    ...
    >>> from plspark.sql.functions import asc, desc
...
def desc(col: "ColumnOrName") -> Column:
    ...
    >>> from plspark.sql.functions import desc
    >>> df = spark.createDataFrame([(4, 'B'), (3, 'A'), (2, 'C')], ['id', 'value'])
...
def try_add(left: "ColumnOrName", right: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    left : :class:`~plspark.sql.Column` or str
    right : :class:`~plspark.sql.Column` or str

    Examples
    --------
    Example 1: Integer plus Integer.

    >>> import plspark.sql.functions as sf
...
def max(col: "ColumnOrName") -> Column:
    ...
    See Also
    --------
    :meth:`plspark.sql.functions.min`
    :meth:`plspark.sql.functions.avg`
    :meth:`plspark.sql.functions.sum`
...
def min(col: "ColumnOrName") -> Column:
    ...
    See Also
    --------
    :meth:`plspark.sql.functions.max`
    :meth:`plspark.sql.functions.avg`
    :meth:`plspark.sql.functions.sum`
...
def max_by(col: "ColumnOrName", ord: "ColumnOrName") -> Column:
    ...
    ord : :class:`~plspark.sql.Column` or str
...
def ceil(col: "ColumnOrName", scale: Optional[Union[Column, int]] = None) -> Column:
    ...
    scale : :class:`~plspark.sql.Column` or int
    ...
    >>> from plspark.sql import functions as sf
...
def bit_get(col: "ColumnOrName", pos: "ColumnOrName") -> Column:
    ...
    col : :class:`~plspark.sql.Column` or str
        target column to compute on.
    pos : :class:`~plspark.sql.Column` or str
        The positions are numbered from right to left, starting at zero.
...
def regr_avgx(y: "ColumnOrName", x: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    y : :class:`~plspark.sql.Column` or str
        the dependent variable.
    x : :class:`~plspark.sql.Column` or str
...
def atan2(col1: Union["ColumnOrName", float], col2: Union["ColumnOrName", float]) -> Column:
    ...
    Parameters
    ----------
    col1 : str, :class:`~plspark.sql.Column` or float
        coordinate on y-axis
    col2 : str, :class:`~plspark.sql.Column` or float
...
def pmod(dividend: Union["ColumnOrName", float], divisor: Union["ColumnOrName", float]) -> Column:
    ...
    Parameters
    ----------
    dividend : str, :class:`~plspark.sql.Column` or float
        the column that contains dividend, or the specified dividend value
    divisor : str, :class:`~plspark.sql.Column` or float
    ...
    Examples
    --------
    >>> from plspark.sql.functions import pmod
    >>> df = spark.createDataFrame([
...
def width_bucket(
    ...
    Parameters
    ----------
    v : str or :class:`~plspark.sql.Column`
        value to compute a bucket number in the histogram
    min : str or :class:`~plspark.sql.Column`
        minimum value of the histogram
    max : str or :class:`~plspark.sql.Column`
        maximum value of the histogram
    numBucket : str, :class:`~plspark.sql.Column` or int
...
def row_number() -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql import Window
    >>> df = spark.range(3)
...
def dense_rank() -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql import Window, types
...
def approx_count_distinct(col: "ColumnOrName", rsd: Optional[float] = None) -> Column:
    """
    This aggregate function returns a new :class:`~plspark.sql.Column`, which estimates
    ...
    See Also
    --------
    :meth:`plspark.sql.functions.count_distinct`

    Examples
    --------
    Example 1: Counting distinct values in a single column DataFrame representing integers

    >>> from plspark.sql.functions import approx_count_distinct
    ...
    >>> from plspark.sql.functions import approx_count_distinct, struct
...
def broadcast(df: DataFrame) -> DataFrame:
    ...
    Returns
    -------
    :class:`~plspark.sql.DataFrame`
        DataFrame marked as ready for broadcast join.

    Examples
    --------
    >>> from plspark.sql import types
    >>> df = spark.createDataFrame([1, 2, 3, 3, 4], types.IntegerType())
...
def coalesce(*cols: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    cols : :class:`~plspark.sql.Column` or str
...
def corr(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    """Returns a new :class:`~plspark.sql.Column` for the Pearson Correlation Coefficient for
    ``col1`` and ``col2``.
    ...
    Parameters
    ----------
    col1 : :class:`~plspark.sql.Column` or str
...
def covar_pop(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    """Returns a new :class:`~plspark.sql.Column` for the population covariance of ``col1`` and
    ``col2``.
...
def covar_samp(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    """Returns a new :class:`~plspark.sql.Column` for the sample covariance of ``col1`` and
    ``col2``.
...
def countDistinct(col: "ColumnOrName", *cols: "ColumnOrName") -> Column:
    """Returns a new :class:`~plspark.sql.Column` for distinct count of ``col`` or ``cols``.
...
def first(col: "ColumnOrName", ignorenulls: bool = False) -> Column:
    ...
    ignorenulls : :class:`~plspark.sql.Column` or str
...
def count_min_sketch(
    ...
    col : :class:`~plspark.sql.Column` or str
        target column to compute on.
    eps : :class:`~plspark.sql.Column` or str
        relative error, must be positive
    confidence : :class:`~plspark.sql.Column` or str
        confidence, must be positive and less than 1.0
    seed : :class:`~plspark.sql.Column` or str
        random seed
...
def nanvl(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    ...
    col2 : :class:`~plspark.sql.Column` or str
...
def percentile(
    ...
    Parameters
    ----------
    col : :class:`~plspark.sql.Column` or str input column.
    percentage : :class:`~plspark.sql.Column`, float, list of floats or tuple of floats
        percentage in decimal (must be between 0.0 and 1.0).
    frequency : :class:`~plspark.sql.Column` or int is a positive numeric literal which
        controls frequency.
...
def percentile_approx(
    ...
    accuracy : :class:`~plspark.sql.Column` or float
...
def struct(
    *cols: Union["ColumnOrName", Union[List["ColumnOrName_"], Tuple["ColumnOrName_", ...]]]
    ...
    Parameters
    ----------
    cols : list, set, str or :class:`~plspark.sql.Column`
        column names or :class:`~plspark.sql.Column`\\s to contain in the output struct.
...
def when(condition: Column, value: Any) -> Column:
    ...
    If :func:`plspark.sql.Column.otherwise` is not invoked, None is returned for unmatched
    conditions.
    ...
    Parameters
    ----------
    condition : :class:`~plspark.sql.Column`
        a boolean :class:`~plspark.sql.Column` expression.
    value :
        a literal value, or a :class:`~plspark.sql.Column` expression.
...
def log(arg1: Union["ColumnOrName", float], arg2: Optional["ColumnOrName"] = None) -> Column:
    ...
    Parameters
    ----------
    arg1 : :class:`~plspark.sql.Column`, str or float
        base number or actual number (in this case base is `e`)
    arg2 : :class:`~plspark.sql.Column`, str or float
...
def any_value(col: "ColumnOrName", ignoreNulls: Optional[Union[bool, Column]] = None) -> Column:
    ...
    ignorenulls : :class:`~plspark.sql.Column` or bool
        if first value is null then look for first non-null value.
...
def histogram_numeric(col: "ColumnOrName", nBins: "ColumnOrName") -> Column:
    ...
    col : :class:`~plspark.sql.Column` or str
        target column to work on.
    nBins : :class:`~plspark.sql.Column` or str
...
def date_format(date: "ColumnOrName", format: str) -> Column:
    ...
    Parameters
    ----------
    date : :class:`~plspark.sql.Column` or str
...
def extract(field: "ColumnOrName", source: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    field : :class:`~plspark.sql.Column` or str
        selects which part of the source should be extracted.
    source : :class:`~plspark.sql.Column` or str
...
def make_date(year: "ColumnOrName", month: "ColumnOrName", day: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    year : :class:`~plspark.sql.Column` or str
        The year to build the date
    month : :class:`~plspark.sql.Column` or str
        The month to build the date
    day : :class:`~plspark.sql.Column` or str
...
def date_add(start: "ColumnOrName", days: Union["ColumnOrName", int]) -> Column:
    ...
    Parameters
    ----------
    start : :class:`~plspark.sql.Column` or str
        date column to work on.
    days : :class:`~plspark.sql.Column` or str or int
...
def datediff(end: "ColumnOrName", start: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    end : :class:`~plspark.sql.Column` or str
...
def date_from_unix_date(days: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    days : :class:`~plspark.sql.Column` or str
...
def add_months(start: "ColumnOrName", months: Union["ColumnOrName", int]) -> Column:
    ...
    start : :class:`~plspark.sql.Column` or str
        date column to work on.
    months : :class:`~plspark.sql.Column` or str or int
...
def months_between(date1: "ColumnOrName", date2: "ColumnOrName", roundOff: bool = True) -> Column:
    ...
    Parameters
    ----------
    date1 : :class:`~plspark.sql.Column` or str
        first date column.
    date2 : :class:`~plspark.sql.Column` or str
        second date column.
...
def to_date(col: "ColumnOrName", format: Optional[str] = None) -> Column:
    """Converts a :class:`~plspark.sql.Column` into :class:`plspark.sql.types.DateType`
    using the optionally specified format. Specify formats according to `datetime pattern`_.
    By default, it follows casting rules to :class:`plspark.sql.types.DateType` if the format
    ...
    -------
    :class:`~plspark.sql.Column`
        date value as :class:`plspark.sql.types.DateType` type.
...
def to_timestamp(col: "ColumnOrName", format: Optional[str] = None) -> Column:
    """Converts a :class:`~plspark.sql.Column` into :class:`plspark.sql.types.TimestampType`
    using the optionally specified format. Specify formats according to `datetime pattern`_.
    By default, it follows casting rules to :class:`plspark.sql.types.TimestampType` if the format
    ...
    -------
    :class:`~plspark.sql.Column`
        timestamp value as :class:`plspark.sql.types.TimestampType` type.
...
def date_trunc(format: str, timestamp: "ColumnOrName") -> Column:
    ...
    timestamp : :class:`~plspark.sql.Column` or str
        input column of values to truncate.
...
def unix_timestamp(
    ...
    Parameters
    ----------
    timestamp : :class:`~plspark.sql.Column` or str, optional
...
def from_utc_timestamp(timestamp: "ColumnOrName", tz: "ColumnOrName") -> Column:
    ...
    tz : :class:`~plspark.sql.Column` or str
        ...
        .. versionchanged:: 2.4
           `tz` can take a :class:`~plspark.sql.Column` containing timezone ID strings.
...
def window(
    ...
    The time column must be of :class:`plspark.sql.types.TimestampType`.
    ...
    and 'end', where 'start' and 'end' will be of :class:`plspark.sql.types.TimestampType`.
    ...
    Parameters
    ----------
    timeColumn : :class:`~plspark.sql.Column`
...
def window_time(
    ...
    Parameters
    ----------
    windowColumn : :class:`~plspark.sql.Column`
...
def session_window(timeColumn: "ColumnOrName", gapDuration: Union[Column, str]) -> Column:
    ...
    'start' and 'end', where 'start' and 'end' will be of :class:`plspark.sql.types.TimestampType`.
    ...
    Parameters
    ----------
    timeColumn : :class:`~plspark.sql.Column` or str
        The column name or column to use as the timestamp for windowing by time.
        The time column must be of TimestampType or TimestampNTZType.
    gapDuration : :class:`~plspark.sql.Column` or str
...
def to_unix_timestamp(
    ...
    format : :class:`~plspark.sql.Column` or str, optional
...
def assert_true(col: "ColumnOrName", errMsg: Optional[Union[Column, str]] = None) -> Column:
    ...
    errMsg : :class:`~plspark.sql.Column` or str, optional
...
def raise_error(errMsg: Union[Column, str]) -> Column:
    ...
    Parameters
    ----------
    errMsg : :class:`~plspark.sql.Column` or str
        A Python string literal or column containing the error message
...
def format_string(format: str, *cols: "ColumnOrName") -> Column:
    ...
    cols : :class:`~plspark.sql.Column` or str
        column names or :class:`~plspark.sql.Column`\\s to be used in formatting
...
def instr(str: "ColumnOrName", substr: str) -> Column:
    ...
    Parameters
    ----------
    str : :class:`~plspark.sql.Column` or str
        target column to work on.
    substr : str
...
def sentences(
    ...
    Parameters
    ----------
    string : :class:`~plspark.sql.Column` or str
        a string to be split
    language : :class:`~plspark.sql.Column` or str, optional
        a language of the locale
    country : :class:`~plspark.sql.Column` or str, optional
...
def locate(substr: str, str: "ColumnOrName", pos: int = 1) -> Column:
    ...
    substr : str
        a string
    str : :class:`~plspark.sql.Column` or str
        a Column of :class:`plspark.sql.types.StringType`
    pos : int, optional
...
def repeat(col: "ColumnOrName", n: Union["ColumnOrName", int]) -> Column:
    ...
    col : :class:`~plspark.sql.Column` or str
        target column to work on.
    n : :class:`~plspark.sql.Column` or str or int
...
def rlike(str: "ColumnOrName", regexp: "ColumnOrName") -> Column:
    ...
    str : :class:`~plspark.sql.Column` or str
        target column to work on.
    regexp : :class:`~plspark.sql.Column` or str
...
def regexp_replace(
    ...
    pattern : :class:`~plspark.sql.Column` or str
        column object or str containing the regexp pattern
    replacement : :class:`~plspark.sql.Column` or str
...
def hex(col: "ColumnOrName") -> Column:
    """Computes hex value of the given column, which could be :class:`plspark.sql.types.StringType`,
    :class:`plspark.sql.types.BinaryType`, :class:`plspark.sql.types.IntegerType` or
    :class:`plspark.sql.types.LongType`.
...
def octet_length(col: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import octet_length
...
def bit_length(col: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import bit_length
    >>> spark.createDataFrame([('cat',), ( '\U0001F408',)], ['cat']) \\
...
def translate(srcCol: "ColumnOrName", matching: str, replace: str) -> Column:
    ...
    Parameters
    ----------
    srcCol : :class:`~plspark.sql.Column` or str
        Source column or strings
    matching : str
        matching characters.
    replace : str
...
def replace(
    ...
    search : :class:`~plspark.sql.Column` or str
        A column of string, If `search` is not found in `str`, `str` is returned unchanged.
    replace : :class:`~plspark.sql.Column` or str, optional
...
def split_part(src: "ColumnOrName", delimiter: "ColumnOrName", partNum: "ColumnOrName") -> Column:
    ...
    delimiter : :class:`~plspark.sql.Column` or str
        A column of string, the delimiter used for split.
    partNum : :class:`~plspark.sql.Column` or str
...
def substr(
    ...
    len : :class:`~plspark.sql.Column` or str, optional
...
def parse_url(
    ...
    Parameters
    ----------
    url : :class:`~plspark.sql.Column` or str
        A column of string.
    partToExtract : :class:`~plspark.sql.Column` or str
        A column of string, the path.
    key : :class:`~plspark.sql.Column` or str, optional
...
def printf(format: "ColumnOrName", *cols: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    format : :class:`~plspark.sql.Column` or str
        string that can contain embedded format tags and used as result column's value
    cols : :class:`~plspark.sql.Column` or str
        column names or :class:`~plspark.sql.Column`\\s to be used in formatting
...
def position(
    ...
    Parameters
    ----------
    substr : :class:`~plspark.sql.Column` or str
        A column of string, substring.
    str : :class:`~plspark.sql.Column` or str
        A column of string.
    start : :class:`~plspark.sql.Column` or str, optional
...
def endswith(str: "ColumnOrName", suffix: "ColumnOrName") -> Column:
    ...
    str : :class:`~plspark.sql.Column` or str
        A column of string.
    suffix : :class:`~plspark.sql.Column` or str
...
def startswith(str: "ColumnOrName", prefix: "ColumnOrName") -> Column:
    ...
    str : :class:`~plspark.sql.Column` or str
        A column of string.
    prefix : :class:`~plspark.sql.Column` or str
...
def btrim(str: "ColumnOrName", trim: Optional["ColumnOrName"] = None) -> Column:
    ...
    str : :class:`~plspark.sql.Column` or str
        Input column or strings.
    trim : :class:`~plspark.sql.Column` or str
...
def elt(*inputs: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    inputs : :class:`~plspark.sql.Column` or str
...
def find_in_set(str: "ColumnOrName", str_array: "ColumnOrName") -> Column:
    ...
    str_array : :class:`~plspark.sql.Column` or str
...
def like(
    ...
    escape : :class:`~plspark.sql.Column`
...
def left(str: "ColumnOrName", len: "ColumnOrName") -> Column:
    ...
    str : :class:`~plspark.sql.Column` or str
        Input column or strings.
    len : :class:`~plspark.sql.Column` or str
...
def create_map(
    *cols: Union["ColumnOrName", Union[List["ColumnOrName_"], Tuple["ColumnOrName_", ...]]]
    ...
    ----------
    cols : :class:`~plspark.sql.Column` or str
        The input column names or :class:`~plspark.sql.Column` objects grouped into
...
def array(
    *cols: Union["ColumnOrName", Union[List["ColumnOrName_"], Tuple["ColumnOrName_", ...]]]
    ...
    ----------
    cols : :class:`~plspark.sql.Column` or str
        Column names or :class:`~plspark.sql.Column` objects that have the same data type.
...
def arrays_overlap(a1: "ColumnOrName", a2: "ColumnOrName") -> Column:
    ...
    Parameters
    ----------
    a1, a2 : :class:`~plspark.sql.Column` or str
...
def slice(
    ...
    start : :class:`~plspark.sql.Column`, str, or int
        The start index for the slice operation. If negative, starts the index from the
        end of the array.
    length : :class:`~plspark.sql.Column`, str, or int
...
def array_join(
    ...
    >>> from plspark.sql import functions as sf
    >>> from plspark.sql.types import StructType, StructField, ArrayType, StringType
...
def concat(*cols: "ColumnOrName") -> Column:
    ...
    See Also
    --------
    :meth:`plspark.sql.functions.array_join` : to concatenate string columns with delimiter
...
def get(col: "ColumnOrName", index: Union["ColumnOrName", int]) -> Column:
    ...
    index : :class:`~plspark.sql.Column` or str or int
...
def array_insert(arr: "ColumnOrName", pos: Union["ColumnOrName", int], value: Any) -> Column:
    ...
    Parameters
    ----------
    arr : :class:`~plspark.sql.Column` or str
...
def explode(col: "ColumnOrName") -> Column:
    ...
    See Also
    --------
    :meth:`plspark.functions.posexplode`
    :meth:`plspark.functions.explode_outer`
    :meth:`plspark.functions.posexplode_outer`
...
def inline(col: "ColumnOrName") -> Column:
    ...
    See Also
    --------
    :meth:`plspark.functions.explode`
    :meth:`plspark.functions.inline_outer`
...
def schema_of_json(json: "ColumnOrName", options: Optional[Dict[str, str]] = None) -> Column:
    ...
    Parameters
    ----------
    json : :class:`~plspark.sql.Column` or str
...
def from_xml(
    ...
    schema : :class:`StructType`, :class:`~plspark.sql.Column` or str
    ...
    --------
    >>> from plspark.sql.types import *
    >>> from plspark.sql.functions import from_xml, schema_of_xml, lit
...
def schema_of_xml(xml: "ColumnOrName", options: Optional[Dict[str, str]] = None) -> Column:
    ...
    Parameters
    ----------
    xml : :class:`~plspark.sql.Column` or str
...
def schema_of_csv(csv: "ColumnOrName", options: Optional[Dict[str, str]] = None) -> Column:
    ...
    Parameters
    ----------
    csv : :class:`~plspark.sql.Column` or str
...
def map_contains_key(col: "ColumnOrName", value: Any) -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import map_contains_key
...
def map_keys(col: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import map_keys
    >>> df = spark.sql("SELECT map(1, 'a', 2, 'b') as data")
...
def map_values(col: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import map_values
    >>> df = spark.sql("SELECT map(1, 'a', 2, 'b') as data")
...
def map_entries(col: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import map_entries
    >>> df = spark.sql("SELECT map(1, 'a', 2, 'b') as data")
...
def map_from_entries(col: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import map_from_entries
...
def array_repeat(col: "ColumnOrName", count: Union["ColumnOrName", int]) -> Column:
    ...
    count : :class:`~plspark.sql.Column` or str or int
...
def arrays_zip(*cols: "ColumnOrName") -> Column:
    ...
    Examples
    --------
    >>> from plspark.sql.functions import arrays_zip
...
def map_concat(
    *cols: Union["ColumnOrName", Union[List["ColumnOrName_"], Tuple["ColumnOrName_", ...]]]
    ...
    ----------
    cols : :class:`~plspark.sql.Column` or str
        column names or :class:`~plspark.sql.Column`\\s
    ...
    Examples
    --------
    >>> from plspark.sql.functions import map_concat
...
def sequence(
    ...
    stop : :class:`~plspark.sql.Column` or str
        last values (inclusive)
    step : :class:`~plspark.sql.Column` or str, optional
...
def from_csv(
    ...
    schema :class:`~plspark.sql.Column` or str
...
def transform(
    ...
    f : function
        ...
        and can use methods of :class:`~plspark.sql.Column`, functions defined in
        :py:mod:`plspark.sql.functions` and Scala ``UserDefinedFunctions``.
...
def exists(col: "ColumnOrName", f: Callable[[Column], Column]) -> Column:
    ...
    f : function
        ...
        Can use methods of :class:`~plspark.sql.Column`, functions defined in
        :py:mod:`plspark.sql.functions` and Scala ``UserDefinedFunctions``.
        Python ``UserDefinedFunctions`` are not supported
        (`SPARK-27052 <https://issues.apache.org/jira/browse/SPARK-27052>`__).
...
def aggregate(
    ...
    Both functions can use methods of :class:`~plspark.sql.Column`, functions defined in
    :py:mod:`plspark.sql.functions` and Scala ``UserDefinedFunctions``.
    Python ``UserDefinedFunctions`` are not supported
    (`SPARK-27052 <https://issues.apache.org/jira/browse/SPARK-27052>`__).
    ...
    col : :class:`~plspark.sql.Column` or str
        name of column or expression
    initialValue : :class:`~plspark.sql.Column` or str
...
def str_to_map(
    ...
    Parameters
    ----------
    text : :class:`~plspark.sql.Column` or str
        Input column or strings.
    pairDelim : :class:`~plspark.sql.Column` or str, optional
        delimiter to use to split pair.
    keyValueDelim : :class:`~plspark.sql.Column` or str, optional
...
def years(col: "ColumnOrName") -> Column:
    ...
    :py:meth:`~plspark.sql.readwriter.DataFrameWriterV2.partitionedBy`
...
def make_dt_interval(
    ...
    hours : :class:`~plspark.sql.Column` or str
        the number of hours, positive or negative
    mins : :class:`~plspark.sql.Column` or str
        the number of minutes, positive or negative
    secs : :class:`~plspark.sql.Column` or str
...
def make_interval(
    ...
    Parameters
    ----------
    years : :class:`~plspark.sql.Column` or str
        the number of years, positive or negative
    months : :class:`~plspark.sql.Column` or str
        the number of months, positive or negative
    weeks : :class:`~plspark.sql.Column` or str
...
def make_timestamp(
    ...
    timezone : :class:`~plspark.sql.Column` or str
...
def call_udf(udfName: str, *cols: "ColumnOrName") -> Column:
    ...
    cols : :class:`~plspark.sql.Column` or str
        column names or :class:`~plspark.sql.Column`\\s to be used in the UDF
    ...
    Examples
    --------
    >>> from plspark.sql.functions import call_udf, col
    >>> from plspark.sql.types import IntegerType, StringType
...
def call_function(funcName: str, *cols: "ColumnOrName") -> Column:
    ...
    cols : :class:`~plspark.sql.Column` or str
        column names or :class:`~plspark.sql.Column`\\s to be used in the function
...
def hll_sketch_agg(col: "ColumnOrName", lgConfigK: Optional[Union[int, Column]] = None) -> Column:
    ...
    Parameters
    ----------
    col : :class:`~plspark.sql.Column` or str or int
...
def hll_union_agg(
    ...
    Parameters
    ----------
    col : :class:`~plspark.sql.Column` or str or bool
...
def nvl2(col1: "ColumnOrName", col2: "ColumnOrName", col3: "ColumnOrName") -> Column:
    ...
    col1 : :class:`~plspark.sql.Column` or str
    col2 : :class:`~plspark.sql.Column` or str
    col3 : :class:`~plspark.sql.Column` or str
...
def aes_encrypt(
    ...
    Parameters
    ----------
    input : :class:`~plspark.sql.Column` or str
        The binary value to encrypt.
    key : :class:`~plspark.sql.Column` or str
        The passphrase to use to encrypt the data.
    mode : :class:`~plspark.sql.Column` or str, optional
        Specifies which block cipher mode should be used to encrypt messages. Valid modes: ECB,
        GCM, CBC.
    padding : :class:`~plspark.sql.Column` or str, optional
        ...
        for CBC.
    iv : :class:`~plspark.sql.Column` or str, optional
    ...
    aad : :class:`~plspark.sql.Column` or str, optional
...
def udf(
    ...
    *,
    ...
    returnType : :class:`plspark.sql.types.DataType` or str
        the return type of the user-defined function. The value can be either a
        :class:`plspark.sql.types.DataType` object or a DDL-formatted type string.
    ...
    Examples
    --------
    >>> from plspark.sql.types import IntegerType
...
def udtf(
    ...
    *,
    ...
    returnType : :class:`plspark.sql.types.StructType` or str, optional
        the return type of the user-defined table function. The value can be either a
        :class:`plspark.sql.types.StructType` object or a DDL-formatted struct type string.
    ...
    ...         yield "hello", "world"
    ...
    >>> from plspark.sql.functions import udtf
    ...
    - Each argument is a :class:`plspark.sql.udtf.AnalyzeArgument`, containing:
      - dataType: DataType
    ...
    and return a :class:`plspark.sql.udtf.AnalyzeResult`, containing.

    - schema: StructType

    >>> from plspark.sql.udtf import AnalyzeArgument, AnalyzeResult
    >>> # or from plspark.sql.functions import AnalyzeArgument, AnalyzeResult
    >>> @udtf
...
def _test() -> None:
    import doctest
    from plspark.sql import SparkSession
    import plspark.sql.functions

    globs = plspark.sql.functions.__dict__.copy()
    spark = SparkSession.builder.master("local[4]").appName("sql.functions tests").getOrCreate()
    sc = spark.sparkContext
    globs["sc"] = sc
    globs["spark"] = spark
    (failure_count, test_count) = doctest.testmod(
        plspark.sql.functions,
        globs=globs,
