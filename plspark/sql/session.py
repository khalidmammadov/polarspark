...
# from build.lib.plspark.sql.functions import struct
from plspark import SparkConf, SparkContext
from plspark.rdd import RDD
# from plspark.sql.column import _to_java_column
...
from plspark.errors.exceptions.captured import install_exception_handler
from plspark.sql.utils import is_timestamp_ntz_preferred, to_str, try_remote_session_classmethod
from plspark.errors import PySparkValueError, PySparkTypeError, PySparkRuntimeError

if TYPE_CHECKING:
    from plspark.sql._typing import AtomicValue, RowLike, OptionalPrimitiveType
    # from plspark.sql.catalog import Catalog
    # from plspark.sql.pandas._typing import ArrayLike, DataFrameLike as PandasDataFrameLike
    # from plspark.sql.streaming import StreamingQueryManager
    # from plspark.sql.udf import UDFRegistration
    # from plspark.sql.udtf import UDTFRegistration
    # from plspark.sql.datasource import DataSourceRegistration

    # Running MyPy type checks will always require pandas and
    # other dependencies so importing here is fine.
    # from plspark.sql.connect.client import SparkConnectClient
...
class SparkSession(SparkConversionMixin):
    ...
    class Builder:
        ...
        def config(
            ...
            *,
            ...
            >>> from plspark.conf import SparkConf
            >>> SparkSession.builder.config(conf=SparkConf())
            <plspark.sql.session.SparkSession.Builder...
        ...
        def getOrCreate(self) -> "SparkSession":
            ...
            True
            """
            from plspark.context import SparkContext
            from plspark.conf import SparkConf
        ...
        #         from plspark.sql.connect.session import SparkSession as RemoteSparkSession
        #
    ...
    def conf(self) -> RuntimeConfig:
        ...
        Returns
        -------
        :class:`plspark.sql.conf.RuntimeConfig`
    ...
    def range(
        ...
        """
        Create a :class:`DataFrame` with single :class:`plspark.sql.types.LongType` column named
    ...
    def _inferSchemaFromList(
        ...
        Returns
        -------
        :class:`plspark.sql.types.StructType`
        """
        if not data:
        ...
        # infer_dict_as_struct = False #self._jconf.inferDictAsStruct()
        # infer_array_from_first_element = False #self._jconf.legacyInferArrayTypeFromFirstElement()
        # prefer_timestamp_ntz = False #is_timestamp_ntz_preferred()
        schema = reduce(
            _merge_type,
            (
                _infer_schema(
    ...
    def _create_shell_session() -> "SparkSession":
        """
        Initialize a :class:`SparkSession` for a plspark shell session. This is called from
    ...
    def createDataFrame(  # type: ignore[misc]
        ...
        schema : :class:`plspark.sql.types.DataType`, str or list, optional
            a :class:`plspark.sql.types.DataType` or a datatype string or a list of
            column names, default is None. The data type string format equals to
            :class:`plspark.sql.types.DataType.simpleString`, except that top level struct type can
            omit the ``struct<>``.
            ...
            When ``schema`` is :class:`plspark.sql.types.DataType` or a datatype string, it must
            match the real data, or an exception will be thrown at runtime. If the given schema is
            not :class:`plspark.sql.types.StructType`, it will be wrapped into a
            :class:`plspark.sql.types.StructType` as its only field, and the field name will be
        ...
        Notes
        -----
        Usage with `spark.sql.execution.arrow.plspark.enabled=True` is experimental.
        ...
        if has_numpy and isinstance(data, np.ndarray):
            ...
            from plspark.sql.pandas.utils import require_minimum_pandas_version
    ...
    def sql(
        ...
        >>> from plspark.sql.functions import create_map
        >>> spark.sql(
        ...
        >>> from plspark.sql.functions import array
        >>> spark.sql(
    ...
    #     --------
    #     >>> spark.readStream
    #     <plspark...DataStreamReader object ...>
    #
    ...
    def streams(self) -> "StreamingQueryManager":
        ...
        --------
        >>> spark.streams
        <plspark...StreamingQueryManager object ...>
    ...
    def stop(self) -> None:
        ...
        """
        from plspark.sql.context import SQLContext
